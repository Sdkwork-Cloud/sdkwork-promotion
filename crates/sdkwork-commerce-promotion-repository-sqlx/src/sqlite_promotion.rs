...
mod tests {
    use sdkwork_commerce_promotion_service::{
        ...
        PromotionOrderCouponBenefitKind, PromotionUserCouponListQuery,
    };
    ...
    async fn sqlite_redeem_promotion_code_credits_points_and_records_coupon_history() {
        ...
        let outcome = store
            .redeem_promotion_code(redeem_command("1", "WELCOME", "redeem-1"))
            .await
            .expect("promotion code redemption");

        assert_eq!("Promotion code redeemed", outcome.message);
        assert_eq!("500", outcome.amount.as_str());
        ...
        let coupons = store
            .list_promotion_user_coupons(
                PromotionUserCouponListQuery::new("100001", Some("300001"), "1", None)
                    .expect("coupon query"),
            )
            .await
            .expect("coupons");
        assert_eq!(1, coupons.len());
        assert_eq!("500", coupons[0].amount.as_str());
        assert_eq!("success", coupons[0].status);

        let coupon_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(1) FROM promotion_user_coupon WHERE tenant_id = '100001' AND subject_id = '1' AND status = 'redeemed'",
        )
        .fetch_one(&pool)
        .await
        ...
        let billing_source_type: String = sqlx::query_scalar(
            "SELECT source_type FROM commerce_billing_history WHERE tenant_id = '100001' AND owner_user_id = '1'",
        )
        .fetch_one(&pool)
        .await
        ...
        let other_coupons = store
            .list_promotion_user_coupons(
                PromotionUserCouponListQuery::new("100001", Some("300001"), "2", None)
    ...
    async fn sqlite_redeem_promotion_code_rejects_duplicate_user_receive() {
        ...
        store
            ...
            .expect("first redeem");
        let error = store
            .redeem_promotion_code(redeem_command("1", "WELCOME", "redeem-2"))
            .await
    ...
    async fn sqlite_redeem_promotion_code_rejects_points_balance_overflow_without_ledger() {
        let pool = migrated_pool().await;
        seed_promotion_codes(&pool).await;
        seed_points_account(&pool, "1", i64::MAX).await;
        let store = super::SqliteCommercePromotionStore::new(pool.clone());

        let error = store
            .redeem_promotion_code(redeem_command("1", "WELCOME", "redeem-overflow"))
            .await
        ...
        let account_balance: String = sqlx::query_scalar(
            "SELECT available_amount FROM commerce_account WHERE tenant_id = '100001' AND owner_user_id = '1' AND asset_type = 'points'",
        )
        .fetch_one(&pool)
        .await
        .expect("account balance");
        let account_ledger_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(1) FROM commerce_account_ledger_entry WHERE tenant_id = '100001' AND owner_user_id = '1'",
        )
        .fetch_one(&pool)
        .await
        .expect("account ledger count");
        let user_coupon_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(1) FROM promotion_user_coupon WHERE tenant_id = '100001' AND owner_user_id = '1'",
        )
        .fetch_one(&pool)
        .await
    ...
    async fn sqlite_redeem_promotion_code_replays_same_idempotency_key_without_duplicate_ledger() {
        ...
        seed_promotion_codes(&pool).await;
        let store = super::SqliteCommercePromotionStore::new(pool.clone());
        let command = redeem_command_with_idempotency("1", "WELCOME", "redeem-1", "idem-1");
    ...
    async fn sqlite_redeem_promotion_code_rejects_idempotency_key_reused_for_different_request() {
        ...
        store
            .redeem_promotion_code(redeem_command_with_idempotency(
                "1", "WELCOME", "redeem-1", "idem-1",
            ))
            .await
            .expect("first redeem");
        let error = store
            .redeem_promotion_code(redeem_command_with_idempotency(
                "1", "OTHER", "redeem-2", "idem-1",
            ))
            .await
