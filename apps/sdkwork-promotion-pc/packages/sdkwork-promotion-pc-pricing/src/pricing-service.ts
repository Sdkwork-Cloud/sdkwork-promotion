...
export function createSdkworkPricingService(
  ...
  return {
    ...
    async getCatalog() {
      ...
      const [walletOverview, subscriptionDashboard, offerDashboard] = await Promise.all([
        walletService.getOverview(),
        subscriptionService.getDashboard(),
        offerService.getDashboard(),
      ]);

      // billing service 失败时降级到空仪表盘，避免拖垮整个 catalog 加载
      const billingDashboard = await billingService
        .getDashboard()
        .catch(() => billingService.getEmptyDashboard());
